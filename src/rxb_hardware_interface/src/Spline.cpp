#include "rxb_hardware_interface/Spline.h"
#include <cstring>
...
//  追赶法解方程求二阶偏导数
    ...
    bool Spline::SinglePointInterp(const double &x, double &y, double &dy)throw(SplineFailure) {
        if((x < MinX) | (x > MaxX)) {
            throw SplineFailure("不支持外插值");
        }
        int klo, khi, k;
        ...
        y = aa * GivenY[klo] + bb * GivenY[khi] + ((aa * aa * aa - aa) * PartialDerivative[klo] + (bb * bb * bb - bb) * PartialDerivative[khi]) * hh * hh / 6.0;
        dy = GivenY[klo] * (-1) / hh + GivenY[khi] / hh + ((3 * aa * aa * (-1) / hh - (-1) / hh) * PartialDerivative[klo] + (3 * bb * bb / hh - 1 / hh) * PartialDerivative[khi]) * hh * hh / 6.0;
        return true;
    }
    bool Spline::SinglePointInterp(const double &x, double &y, double &dy, double &ddy)throw(SplineFailure) {
        if((x < MinX) | (x > MaxX)) {
            throw SplineFailure("不支持外插值");
        }
        int klo, khi, k;
        ...
        dbb = 1 / hh;
        y = aa * GivenY[klo] + bb * GivenY[khi] + ((aa * aa * aa - aa) * PartialDerivative[klo] + (bb * bb * bb - bb) * PartialDerivative[khi]) * hh * hh / 6.0;
        dy = GivenY[klo] * daa + GivenY[khi] * dbb + ((3 * aa * aa * daa - daa) * PartialDerivative[klo] + (3 * bb * bb * dbb - dbb) * PartialDerivative[khi]) * hh * hh / 6.0;
        ddy = (6 * aa * daa * daa  * PartialDerivative[klo] + 6 * bb * dbb * dbb  * PartialDerivative[khi]) * hh * hh / 6.0;
        return true;
    }
...
*/
    ...
    bool Spline::MultiPointInterp(const double *x, const int &num, double *y, double *dy, double *ddy)throw(SplineFailure) {
        for(int i = 0; i < num; i++) {
            SinglePointInterp(x[i], y[i], dy[i], ddy[i]);
        }
        return true;
    }
    //自动多个插值的实现
